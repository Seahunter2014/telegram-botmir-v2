...
def rating_keyboard_from_report(report) -> InlineKeyboardMarkup | None:
    publication = report._step_details("publication.summary") if hasattr(report, "_step_details") else None
    ...
    post_id = str(ids[0])
    rows = []
    rows.append([InlineKeyboardButton(f"⭐ {i}", callback_data=f"rate:{post_id}:{i}") for i in range(1, 6)])
    rows.append([InlineKeyboardButton(f"⭐ {i}", callback_data=f"rate:{post_id}:{i}") for i in range(6, 11)])
    return InlineKeyboardMarkup(rows)


def publication_title_by_post_id(post_id: str) -> str:
    for item in reversed(state.publications()):
        if str(item.get("post_id")) == str(post_id):
            return str(item.get("title") or "")
    return ""


async def save_rating_message(message, post_id: str, score: int, source: str = "manual") -> None:
    title = publication_title_by_post_id(post_id)
    item = state.record_post_rating(post_id, score, title=title, source=source)
    label = "хорошая оценка — усилим похожую подачу" if score >= 8 else ("низкая оценка — бот будет избегать похожей подачи" if score <= 6 else "нейтральная оценка сохранена")
    await message.reply_text(f"Оценка сохранена: пост {post_id} — {score}/10. {label}", reply_markup=main_menu())


async def parse_and_save_rating(message, raw: str, source: str = "manual") -> None:
    parts = [x.strip() for x in raw.replace(",", " ").split() if x.strip()]
    if len(parts) < 2:
        await message.reply_text("Введите: номер поста и оценку 1–10. Пример: 606 9")
        return
    post_id = parts[0].replace("https://t.me/NadoTurKrd/", "").strip()
    try:
        score = int(parts[1])
    except Exception:
        await message.reply_text("Оценка должна быть числом от 1 до 10.")
        return
    if not 1 <= score <= 10:
        await message.reply_text("Оценка должна быть от 1 до 10.")
        return
    await save_rating_message(message, post_id, score, source=source)
...
async def run_once_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await send_long(update.effective_message, report.admin_text())
    rk = rating_keyboard_from_report(report)
    if rk:
        await update.effective_message.reply_text("Оцените опубликованный пост от 1 до 10:", reply_markup=rk)
...
async def rate_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    raw = " ".join(context.args).strip()
    if not raw:
        context.user_data["awaiting"] = "rate_post"
        await update.effective_message.reply_text("Введите номер поста и оценку 1–10. Пример: 606 9")
        return
    await parse_and_save_rating(update.effective_message, raw, source="command")
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if data.startswith("publish:"):
        ...
        rk = rating_keyboard_from_report(report)
        if rk:
            await q.message.reply_text("Оцените опубликованный пост от 1 до 10:", reply_markup=rk)
    elif data.startswith("rate:"):
        _, post_id, score_s = data.split(":", 2)
        await save_rating_message(q.message, post_id, int(score_s), source="button")
        await q.edit_message_text(f"Оценка сохранена: пост {post_id} — {score_s}/10")
...
async def menu_text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if awaiting == "set_channels":
        ...
        return
    if awaiting == "rate_post":
        await parse_and_save_rating(update.effective_message, text, source="menu")
        return
...
async def scheduled_autopost(app: Application) -> None:
    ...
    if settings.telegram_admin_id:
        try:
            ...
            rk = rating_keyboard_from_report(report)
            if rk:
                await app.bot.send_message(chat_id=int(settings.telegram_admin_id), text="Оцените опубликованный пост от 1 до 10:", reply_markup=rk)
        except Exception:
...
def build_app() -> Application:
    ...
    commands = [
        ...
        ("topics", topics_cmd), ("why_skipped", why_skipped_cmd), ("last", last_cmd), ("rate", rate_cmd),
    ]
